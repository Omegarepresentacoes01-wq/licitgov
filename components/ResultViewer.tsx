...
export const ResultViewer: React.FC<ResultViewerProps> = ({ content, isGenerating, documentTitle }) => {
  ...
  if (!content && !isGenerating) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-10 transition-colors duration-300">
        ...
        <p className="text-xl font-semibold text-slate-600 dark:text-slate-400">Pronto para criar?</p>
        <p className="text-sm mt-2 text-center max-w-sm">Selecione o tipo de documento, preencha as informações e a IA gerará um esboço completo para você.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-slate-800 overflow-hidden relative transition-colors duration-300">
      <div className="bg-white dark:bg-slate-900 p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
        ...
        <div className="flex gap-2">
            ...
            <button 
                onClick={handleCopy}
                disabled={!content}
                className="text-xs font-medium bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-lg transition-colors disabled:opacity-50 flex items-center gap-2"
            >
            ...
            <button 
                ...
                disabled={!content}
                className="text-xs font-medium bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors disabled:opacity-50 flex items-center gap-2 shadow-sm"
            >
      ...
      <div className="flex-1 overflow-y-auto p-8 bg-slate-50 dark:bg-slate-950/50">
        <div className="max-w-4xl mx-auto shadow-sm min-h-[600px] border border-slate-200 dark:border-slate-800 rounded-xl p-10 bg-white dark:bg-slate-900 transition-colors duration-300">
